...
public class AssignmentTypesConstructorExpressionsTest_6 {
	...
	public void assignmentExpressionsTest() {
		...
		// definir una expresion que asigne y obtenga a la vez el nombre 'Nikola Tesla' sobre el nombre del Inventor Tesla
		teslaContext.setVariable("x", teslaNameExpected);
		String teslaName3 = spelParser.parseExpression("name = #x").getValue(teslaContext, String.class);
	...
	public void typesExpressionsTest() {
		...
		// definir una expresion que calcule el area de un circulo de 5.5 de radio (multiplicar y dividir por 1000.0 para truncar decimales
		String area = "T(Math).round( T(Math).PI * T(Math).pow('5.5', '2') * 10000.0 ) / 10000.0";
		Double circleArea = spelParser.parseExpression(area).getValue(Double.class);
	...
	public void contructorExpressionsTest() {
		...
		String newInventorString = "new org.certificatic.spring.core.practica21.spel.model.Inventor("
				+ "'Albert Einstein', #birthday.getTime() ,'German')";
		Inventor inventor = spelParser.parseExpression(newInventorString).getValue(context, Inventor.class);
		...
		String[] stringArray = spelParser.parseExpression("new String[] {'absorption refrigerator', 'quantum theory', 'theory of general relativity'}").getValue(String[].class);
